function initializeBelongsToOneHasOneControl(event) {
  $(".belongs-to-one-container").each(function(index, element) {
    var modelId = $(element).data("model-id");

    if (modelId) {
      $(element)
        .find(".delete-form-fields")
        .removeClass("hidden");
    } else {
      $(element)
        .find(".add-form-fields")
        .removeClass("hidden");
    }
  });
  ...
  $(document).on("click", ".add-form-fields", function(e) {
    ...
    if (fieldsContainer.length == 0) {
      ...
      var editableAttributes = $(domElement)
        .find(".editable-attributes")
        .html();
